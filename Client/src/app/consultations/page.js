...
const Page = () => {
    ...
    useEffect(() => {
        if (!userLoggedIn)
            router.replace(`/login?returnTo=/consultations`)
        else
            (async () => {
                try {
                    let res = await axios.get(`/api/consultations`, { withCredentials: true })
...
function DisplayConsultation({ consultation }) {
    ...
    return (
        <>
            <Link href={`/consult/${consultation.consultationId}`}>
                <div className='cursor-pointer border-2 mb-2 p-2'>
                    <div>{consultation.docid.name}</div>
                    {lastMessage &&
                        <div className={`text-right ${consultation.numberOfUnreadMessagesPatient > 0 && "font-bold"}`}>
                            {consultation.numberOfUnreadMessagesPatient > 0 && <p className='text-center'>New message</p>}
